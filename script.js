...
const circus = (cityName)=>{
        ...
        .then((response) => {
            ...
            document.getElementById('tempc').innerHTML = `Temperature (&deg c) : ${response.current.temp_c}`;
            document.getElementById('temp').innerHTML = `<h3>Temperature : ${response.current.temp_c}</h3>`;        
            document.getElementById('tempf').innerHTML = `Temperature (&deg f) : ${response.current.temp_f}`;
            document.getElementById('humid').innerHTML = `Humidity : ${response.current.humidity}`;
            document.getElementById('feelc').innerHTML = `Feels Like (&deg c) :: ${response.current.feelslike_c}`;
            document.getElementById('feelf').innerHTML = `Feels Like (&deg f) : ${response.current.feelslike_f}`;
